...
def extract(code):
    regx= re.compile(r'\s*LIBNAME\s+\w+\s+.*\s*;', re.IGNORECASE)
    total_libname_counts= len(re.findall(regx, code))
    return [{
        'name':'total_libname_count',
        'desc':'Total number of Libname Statements',
        'value': total_libname_counts}
        ]
